...
# Create the window and scale it to the screen resolution
...
# Left hand and right HandArrows and up arrow
    ...
    for trial in trials:  # trial = +1 (extension)/-1(flexion)
        ...
        # sendTiD(768)  # send Event CUE: 768 to LOOP to indicate start of new trial
        ...
        # refill rectangles to cover edges in center
        # pygame.draw.rect(screen, bigRecColor,
        #                  (Down_rectangle_X + 2, Down_rectangle_Y + Bigthickness,
        #                   Bigger_rectangle_height - 4,
        #                   Bigger_rectangle_height - 6),
        #                  0)
        # pygame.draw.rect(screen, bigRecColor,
        #                  (Bigger_rectangle_X + Bigthickness, Bigger_rectangle_Y + 2,
        #                   Bigger_rectangle_width - 6,
        #                   Bigger_rectangle_height - 4),
        #                  0)
        pygame.display.update()
        ...
        # # Draw the fixxation cue
        # if fixationCrossTime > 0:
        #     pygame.draw.rect(screen, white, (
        #     centerOfScreen[0] - bar_height / 3, centerOfScreen[1] - bar_height / 15, 2 * bar_height / 3, 2 * bar_height / 15),
        #                      0)  # horizontal rectangle for cross cue
        #     pygame.draw.rect(screen, white, (
        #     centerOfScreen[0] - bar_height / 15, centerOfScreen[1] - bar_height / 3, 2 * bar_height / 15, 2 * bar_height / 3),
        #                      0)  # Vertical rectangle for cross cue
        #     pygame.display.update()

        #     time.sleep(fixationCrossTime)  # wait for the period determined in a0_configFile.py

        # # over ride the fixation cue to hide it with the bar
        # pygame.draw.rect(screen, barColor, (initial_x_new, initial_y, bar_width, bar_width), thickness)  # Moving bar specs
        ...
        # COMENTAMOS ESTO
        #    if (currentClass == -1):
        #        sendTiD(769)  # send Event CUE: 769 to LOOP to indicate flexion cue
        #    if (currentClass == 1):
        #        sendTiD(770)  # send Event CUE: 770 to LOOP to indicate extension cue
        #    if (currentClass == 2):
        #        sendTiD(771)
        ...
        while True:
            ...
            #
            #        if type(message[0])==type(1.0):
            #            prob=message
            #            # fprob.write('%07.3f' % (time.time() - prevTime) + '\t')
            #            # for p in prob:
            #                # fprob.write('%05.4f' % (p/100) + '\t')
            #            # fprob.write('\n')
            #
            #        pec_val = prob[0] / 100
            #        trap_val = prob[1] / 100
            #
            #        if prob[0] > 1 or prob[1] > 1: # detection of movement onset
            #            sendTiD(8002)
            ...
            if (time.time() - prevTime >= taskTime):
                ...
                #            if currentClass == -1:
                #                sendTiD(7692)  # send Event CUE: 7692 to LOOP to indicate end of flexion task and start of result view time
                #            elif currentClass == 1:
                #                sendTiD(7702)  # send Event CUE: 7702 to LOOP to indicate end of extension task and start of result view time
                #            elif currentClass == 2:
                #                sendTiD(7712)
...
# Commented:
# COMENTAMOS ESTO
# sendTiD(32766)  # send Event CUE: 32766 agian to LOOP to indicate end of the run
    time.sleep(3)
    logFile = sys.argv[1]
    print(logFile)
    f = open(logFile, "a+")
